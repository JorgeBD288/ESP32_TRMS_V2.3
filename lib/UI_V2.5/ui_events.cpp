...
// Variable para la temporización de la muestra de mensaje de comprobación de guardado y carga
extern float temp_Save_Message = 0.0f;
extern float temp_Load_Message = 0.0f;
...
{
    int32_t value = lv_slider_get_value(ui_ControlDeBrillo);

    if (value < 1) value = 1;
    if (value > 100) value = 100;

    uint8_t pwm = (uint8_t)((value * 255) / 100);
    ledcWrite(BRILLO_PWM_CHANNEL, pwm);

    Serial.printf("[BRILLO] value=%ld  PWM=%u\n", value, pwm);
}

void function_Brightness_Control(lv_event_t * e)
{
    (void)e;              // para evitar warning “unused”
    brightness_Control(); // lee el slider global
}
...
{   //Actualización de elementos visibles y del os registros de los motores
    ...
    //Actualización del control de motores
...
void function_Save_Config_1(lv_event_t * e)
{
    ...
    lv_obj_clear_flag(ui_Label36, LV_OBJ_FLAG_HIDDEN);
    temp_Save_Message = millis();
    flag_Save_Message = true;
}
...
void Show_Save_Message_Selected(){
    lv_obj_add_flag(ui_Label36, LV_OBJ_FLAG_HIDDEN);
    flag_Save_Message = false;
}
